...
<?php
    ...
    <?php while ( $wp_query->have_posts() ) : $wp_query->the_post(); sltws_post_meta(); ?>
        ...
        <?php $theThumbnail = get_the_post_thumbnail(); ?>
	      <div class="blogRow" id="blogPost_<?php echo $counter; ?>"/>
	        <span class="blogTitle"><a href="<?php the_permalink(); ?>"><?php the_title();?></a></span>
	         <span class="blogCopy"><?php the_content();?></span>
        </div>
        <?php endwhile; ?>
        </div>
